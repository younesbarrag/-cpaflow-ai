...
@if ($message || session($type))
    @php
        $flashMessage = $message ?? session($type);
        $flashType = $type;
        $typeClasses = match($flashType) {
    ...
    <div
        ...
        class="rounded-lg border p-4 mb-6 {{ $typeClasses }}"
        role="alert"
    >
        <div class="flex items-center justify-between">
            ...
            <button
                @click="show = false"
                class="ml-4 text-current opacity-50 hover:opacity-75 transition-opacity"
                aria-label="Dismiss"
            >
